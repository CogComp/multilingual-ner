...
@DatasetReader.register("my_text_anno")
class TextAnnotationDatasetReader(DatasetReader):
    ...
    def __init__(self,
                 ...
                 files_ratio: float = 0.5,
                 files_threshold: int = 0,
        ...
        self.files_ratio = files_ratio
        self.files_threshold = files_threshold

    @overrides
    def _read(self, file_paths: str) -> Iterable[Instance]:
        # ruohao: for multilingul, put all files into a list, shuffle, then read
        file_list = []
        print("\n=====  file path  =====\n", file_paths)
        ...
        # truncate if needed:
        if self.limit_files:
            thrd = int(len(file_list) * self.files_ratio)
            if self.files_threshold != 0:
                print("self.files_threshold")
                file_list = file_list[:self.files_threshold]
            else:
                print("\nratio\n")
                file_list = file_list[:thrd]

            print("after truncating:  count = ", len(file_list))

        for fname in file_list:
            doc = ccg_nlpy.load_document_from_json(fname)
        ...
        for file_path in file_paths.split(","):
            ...
            for fname in fnames:
                #print("fname = ", fname)
                doc = ccg_nlpy.load_document_from_json(file_path + "/" + fname)
                label_indices = ["O"] * len(doc.tokens)
                if "NER_CONLL" in doc.view_dictionary:
                    ner = doc.get_ner_conll
                if "NER_ONTONOTES" in doc.view_dictionary:
